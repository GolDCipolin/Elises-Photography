...
<table class="tbl-full sale">
    <a href="high_val.php" class="btn-primary sa">Sort by Highest Price</a>
    ...
    <?php
        $sql_sum="SELECT SUM(final_price) AS totalPrice FROM orders";
        ...
        $totalPrice=$row_sum['totalPrice'];
        ?>
        <td class="sum">Overall Total Price £<?php echo $totalPrice; ?></td>
        <?php
    ?>
    <tr>
        <th>Order Num</th>
                ...
                if($count2){
                    while($row2=mysqli_fetch_assoc($res2)){
                        ...
                        <tr>
                            ...
                            <!-- <td>
                                <a href="view-order.php?id=<?php echo $order_num; ?>" class="btn-secondary">View Details</a>
                            </td> -->
                        </tr>
                        <?php
                    }
...
<!-- <table class="sort">
    <?php
        $sql_sort="SELECT * FROM orders ORDER BY final_price DESC";
        $conn2 = mysqli_connect(DB_HOST, DB_SELECTER, DB_SEL_PASS, DB_NAME);
        $res_sort=mysqli_query($conn2, $sql_sort);
        $count2 = mysqli_num_rows($res_sort);
        if($count2){
            while($row3=mysqli_fetch_assoc($res_sort)){
                ?>
                    <tr>
                        <td><?php echo $order_num; ?></td>
                        <td><?php echo $order_date; ?></td>
